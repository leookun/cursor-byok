...
func resolveModelAdapterChannel(adapters []ModelAdapterConfig, requestedModel string) (*legacyruntime.ResolvedChannel, error) {
	...
	resolved := &legacyruntime.ResolvedChannel{
		...
		SystemPromptEnabled:         matched.SystemPromptEnabled,
		SystemPrompt:                strings.TrimSpace(matched.SystemPrompt),
		SystemPromptPosition:        strings.TrimSpace(matched.SystemPromptPosition),
