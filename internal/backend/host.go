...
func (host *Host) rebuildLocked(cfg serverconfig.Config) error {
	...
	host.mux = server.New(
		...
		server.POST(forwarder.AgentServiceUpdateConversationMetadataProcedure,
			server.Name("agent_update_conversation_metadata"),
			server.ConnectUnary(),
			server.Local(server.HTTPHandlerAction(agentModule.AgentHandler)),
			server.Upstream(upstream.DirectAction(routeDeps, upstream.CompatRouteConfig{
				Name: "agent_update_conversation_metadata",
			})),
		),
