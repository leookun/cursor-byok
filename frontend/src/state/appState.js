...
const GENERIC_SERVICE_ERROR = localized("a1b2c3d4e5f6006b", "Service Error").toString();
...
export const ROUTE_MODE_OPTIONS = [
  { label: localized("3bf8512aa520ed21", "Local Service Mode"), value: "local" },
  { label: localized("699fe7ade5407687", "Direct Mode"), value: "upstream" },
];
...
function formatReleaseDate(value) {
  const text = asString(value);
  if (!text) {
    return localized("4d8c1c5b42830791", "Unknown").toString();
  }
...
export function formatModelAdapterTestSummary(source) {
  ...
  if (status === "running") {
    return localized("a1b2c3d4e5f6003b", "Testing...").toString();
  }
  if (status === "error") {
    return asString(result.error) || localized("a1b2c3d4e5f6008f", "Model test failed").toString();
  }
  if (status !== "success") {
    return "";
  }
  const roundedTPS = Math.max(0, Math.round(asNumber(result.tokensPerSecond)));
  return `${roundedTPS} t/s | ${localized("a1b2c3d4e5f60094", "First token").toString()} ${formatDuration(result.firstTextTokenMS)}`;
}
...
function normalizeModelAdapterTestResult(source) {
  ...
  if (status === "error" && !normalized.summaryText) {
    normalized.summaryText = normalized.error || localized("a1b2c3d4e5f6008f", "Model test failed").toString();
  }
  return normalized;
}
...
export function createEmptyModelAdapter() {
  return {
    ...
    baseURL: "",
    apiKey: "",
    tooltipData: localized("daede9881787abe7", "Notes").toString(),
    modelID: "",
...
function validateJSONObject(value, label) {
  const text = asString(value);
  if (!text) {
    return `${label} ${localized("a1b2c3d4e5f600a1", "cannot be empty").toString()}`;
  }
  try {
    const parsed = JSON.parse(text);
    if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) {
      return `${label} ${localized("a1b2c3d4e5f600a2", "must be a JSON object").toString()}`;
    }
  } catch (_error) {
    return `${label} ${localized("a1b2c3d4e5f600a3", "must be a valid JSON object").toString()}`;
  }
  return "";
}

function validateHeadersJSON(value) {
  const objectError = validateJSONObject(value, localized("a1b2c3d4e5f6004e", "Custom Headers JSON").toString());
  if (objectError) {
    return objectError;
  }
  ...
  for (const [key, item] of Object.entries(parsed)) {
    if (!asString(key)) {
      return localized("a1b2c3d4e5f600a4", "Custom header name cannot be empty").toString();
    }
    if (typeof item !== "string") {
      return `${localized("a1b2c3d4e5f600a5", "Custom header").toString()} ${key} ${localized("a1b2c3d4e5f600a6", "value must be a string").toString()}`;
    }
  }
  return "";
}
...
function validateOpenAIExtraParamsJSON(value) {
  return validateJSONObject(value, localized("a1b2c3d4e5f6004c", "Extra Params JSON").toString());
}

function validateAnthropicExtraParamsJSON(value) {
  return validateJSONObject(value, localized("a1b2c3d4e5f60062", "Anthropic Extra Params JSON").toString());
}
...
export function validateModelAdapters(source) {
  ...
  for (const [index, adapter] of adapters.entries()) {
    const prefix = `${localized("c98e118e0a43f078", "Model").toString()} ${index + 1}`;
    if (!adapter.displayName) {
      return `${prefix}: ${localized("a1b2c3d4e5f60043", "Display Name").toString()} ${localized("a1b2c3d4e5f600a1", "cannot be empty").toString()}`;
    }
    if (!SUPPORTED_MODEL_ADAPTER_TYPES.has(adapter.type)) {
      return `${prefix}: ${localized("a1b2c3d4e5f60050", "Type").toString()} ${localized("a1b2c3d4e5f600a7", "only supports OpenAI or Anthropic").toString()}`;
    }
    if (!adapter.baseURL) {
      return `${prefix}: ${localized("a1b2c3d4e5f60046", "Base URL").toString()} ${localized("a1b2c3d4e5f600a1", "cannot be empty").toString()}`;
    }
    if (!adapter.apiKey) {
      return `${prefix}: ${localized("a1b2c3d4e5f60045", "API Key").toString()} ${localized("a1b2c3d4e5f600a1", "cannot be empty").toString()}`;
    }
    if (!adapter.tooltipData) {
      return `${prefix}: ${localized("a1b2c3d4e5f6004f", "Notes").toString()} ${localized("a1b2c3d4e5f600a1", "cannot be empty").toString()}`;
    }
    if (!adapter.modelID) {
      return `${prefix}: ${localized("a1b2c3d4e5f60044", "Model ID").toString()} ${localized("a1b2c3d4e5f600a1", "cannot be empty").toString()}`;
    }
    if (adapter.type === "openai" && !SUPPORTED_REASONING_EFFORTS.has(adapter.reasoningEffort)) {
      return `${prefix}: ${localized("a1b2c3d4e5f60048", "Reasoning Effort").toString()} ${localized("a1b2c3d4e5f600a8", "only supports low, medium, high, xhigh").toString()}`;
    }
    if (adapter.type === "openai" && !isValidOpenAIEndpoint(adapter.openAIEndpoint)) {
      return `${prefix}: ${localized("a1b2c3d4e5f6004b", "Endpoint").toString()} ${localized("a1b2c3d4e5f600a9", "only supports /v1/responses, /v1/chat/completions or custom path starting with /").toString()}`;
    }
    ...
    if (adapter.type === "anthropic" && !SUPPORTED_ANTHROPIC_THINKING_EFFORTS.has(adapter.anthropicThinkingEffort)) {
      return `${prefix}: ${localized("a1b2c3d4e5f6004a", "Thinking Effort").toString()} ${localized("a1b2c3d4e5f600aa", "only supports low, medium, high, xhigh, max").toString()}`;
    }
    if (adapter.contextWindowTokens && (!Number.isInteger(adapter.contextWindowTokens) || adapter.contextWindowTokens <= 0)) {
      return `${prefix}: ${localized("a1b2c3d4e5f60047", "Context Window").toString()} ${localized("a1b2c3d4e5f600ab", "must be a positive integer").toString()}`;
    }
    if (adapter.maxCompletionTokens && (!Number.isInteger(adapter.maxCompletionTokens) || adapter.maxCompletionTokens <= 0)) {
      return `${prefix}: ${localized("a1b2c3d4e5f60049", "Max Output Tokens").toString()} ${localized("a1b2c3d4e5f600ab", "must be a positive integer").toString()}`;
    }
    ...
    if (adapter.thinkingBudgetTokens && (!Number.isInteger(adapter.thinkingBudgetTokens) || adapter.thinkingBudgetTokens <= 0)) {
      return `${prefix}: ${localized("2caeaec539e78898", "Thinking Budget Token").toString()} ${localized("a1b2c3d4e5f600ab", "must be a positive integer").toString()}`;
    }
    const dedupeKey = buildModelAdapterIdentityKey(adapter);
    if (seenIdentityKeys.has(dedupeKey)) {
      return localized("0e40a09ab4e664ab", "Duplicate model channel detected. Check the combination of url, modelID, apiKey, and displayName").toString();
    }
...
function validateConfigPayload(payload) {
  if (!SUPPORTED_ROUTE_MODES.has(normalizeRouteMode(payload?.routing?.mode, ""))) {
    return localized("1af38868896cf53d", "Routing mode only supports local or upstream").toString();
  }
  return "";
}
...
export const appViewState = reactive({
  serviceStatusText: computed(() => {
    if (appState.proxyRunning && appState.backendRunning) {
      return localized("86df7ec743047234", "Service running").toString();
    }
    if (appState.backendRunning) {
      return localized("65cc5fd2e6ce6e75", "Backend started, proxy not started").toString();
    }
    return localized("26a3855aed1d8d17", "Service not running").toString();
  }),
  ...
  serviceButtonText: computed(() => {
    if (appState.serviceBusy) {
      return appState.serviceRunning ? localized("fb7a4c81729ed0ca", "Stopping...").toString() : localized("ca00a39fcea70dc6", "Starting...").toString();
    }
    return appState.serviceRunning ? localized("f474a4108aba4c4c", "Stop Service").toString() : localized("18b7312022cd1840", "Start Service").toString();
  }),
});
...
export const updateViewState = reactive({
  ...
  promptTitle: computed(() => {
    switch (appState.updatePromptKind) {
      case "ready":
        return localized("ac217e4d1ca410f1", "New version available").toString();
      case "error":
        return localized("ec99e5c45d648fd6", "Update failed").toString();
      default:
        return localized("7f68ebad19ba6bcd", "Check for Updates").toString();
    }
  }),
  promptContent: computed(() => {
    switch (appState.updatePromptKind) {
      case "ready":
        return [
          `${localized("dc82c5e8fb2ab777", "Version: v{0}").toString(appState.updateVersion || appState.appVersion || "...")}`,
          `${localized("3ea83f9f55062582", "Release date: {0}").toString(formatReleaseDate(appState.updateReleaseDate))}`,
          "",
          appState.updateReleaseNotes || localized("02216368edc68816", "No release notes").toString(),
        ].join("\n");
      case "error":
        return appState.updateError || appState.updateMessage || GENERIC_SERVICE_ERROR;
      default:
        return appState.updateMessage || localized("a1a038dfa16c3ede", "You're already on the latest version (v{0}).").toString(appState.appVersion || "...");
    }
  }),
  promptConfirmText: computed(() =>
    appState.updatePromptKind === "ready" ? localized("a1b2c3d4e5f60087", "Restart Now to Update").toString() : localized("fac2a67ad87807c4", "OK").toString(),
  ),
  promptCancelText: computed(() =>
    appState.updatePromptKind === "ready" ? localized("9d2ca261281a158a", "Later").toString() : localized("2cd0f3be8738a86c", "Cancel").toString(),
  ),
...
export async function deleteModelAdapterAt(index) {
  ...
  if (index < 0 || index >= nextAdapters.length) {
    return {
      ok: false,
      error: localized("a1b2c3d4e5f6006e", "Model settings not found; cannot delete").toString(),
    };
  }
...
function splitDisplayNameSeed(value) {
  ...
  if (!match) {
    return { base: text || localized("c98e118e0a43f078", "Model").toString(), number: 0 };
  }
  const base = asString(match[1]) || localized("c98e118e0a43f078", "Model").toString();
...
export async function duplicateModelAdapterAt(index) {
  ...
  if (index < 0 || index >= nextAdapters.length) {
    return {
      ok: false,
      error: localized("a1b2c3d4e5f60070", "Model settings not found; cannot duplicate").toString(),
    };
  }
  ...
  const duplicate = {
    ...source,
    id: "",
    displayName: buildNextDisplayName(nextAdapters, source.displayName || source.modelID || localized("c98e118e0a43f078", "Model").toString()),
  };
...
export async function startService() {
  if (appState.serviceBusy) {
    return { ok: false, error: localized("986678eccf56dc28", "Service status is being updated. Please try again later.").toString() };
  }
